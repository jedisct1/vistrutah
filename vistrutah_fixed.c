// Vistrutah Block Cipher - Corrected Implementation
...
// Corrected key expansion following specification
static void vistrutah_256_key_expansion_fixed(const uint8_t* key, int key_size,
    ...
    for (int i = 0; i <= rounds; i++) {
        if (i % 2 == 0) {
            // Even rounds: use fixed key K0 (with inverse MixColumns for decryption)
            ks->round_keys[i] = K0;
            ks->inv_round_keys[i] = vaesimcq_u8(K0);  // MC^-1(K0) for decryption
        } else {
            ...
            if ((i / 2) % 2 == 0) {
                ...
                ks->inv_round_keys[i] = veorq_u8(k0, rc);  // Same for decryption
                ...
                ks->inv_round_keys[i] = veorq_u8(k1, rc);  // Same for decryption
...
// Corrected Vistrutah-256 encryption following specification exactly
void vistrutah_256_encrypt_fixed(const uint8_t* plaintext, uint8_t* ciphertext,
    ...
    // Key expansion
    vistrutah_256_key_expansion_fixed(key, key_size, &ks, rounds);
    ...
    for (int round = 0; round < rounds; round++) {
        if (round == rounds - 1) {
            ...
            state.slice[0] = vaeseq_u8(state.slice[0], ks.round_keys[round]);
            state.slice[1] = vaeseq_u8(state.slice[1], ks.round_keys[round]);
            // No MixColumns for final round
            state.slice[0] = veorq_u8(state.slice[0], vdupq_n_u8(0));
            state.slice[1] = veorq_u8(state.slice[1], vdupq_n_u8(0));
        } else {
            // Regular round
            state.slice[0] = vaeseq_u8(state.slice[0], ks.round_keys[round]);
            state.slice[1] = vaeseq_u8(state.slice[1], ks.round_keys[round]);
...
// Corrected Vistrutah-256 decryption
void vistrutah_256_decrypt_fixed(const uint8_t* ciphertext, uint8_t* plaintext,
    ...
    for (int round = rounds - 1; round >= 0; round--) {
        // Apply inverse mixing layer before rounds (except first)
        if ((round % 2 == 1) && (round < rounds - 1)) {
            vistrutah_256_inv_mix(&state);
        }
        
        // Determine which round key to use
        uint8x16_t round_key;
        if (round % 2 == 0) {
            // Even round: use inverse MixColumns of fixed key for decryption
            round_key = ks.inv_round_keys[round];
        } else {
            // Odd round: use same variable key as encryption
            round_key = ks.inv_round_keys[round];
        }
        
        if (round == rounds - 1) {
            // Inverse final round (no InvMixColumns)
            state.slice[0] = vaesdq_u8(state.slice[0], round_key);
            state.slice[1] = vaesdq_u8(state.slice[1], round_key);
        } else {
...
// Test function to verify specification compliance
#include <stdio.h>

void test_vistrutah_256_fixed() {
    printf("\n=== Testing Corrected Vistrutah-256 Implementation ===\n");
    ...
    vistrutah_256_encrypt_fixed(plaintext, ciphertext, key, 32, 10);
    vistrutah_256_decrypt_fixed(ciphertext, decrypted, key, 32, 10);
    ...
    // Display key schedule details
    printf("\nKey Schedule Analysis (first 5 rounds):\n");
    vistrutah_key_schedule_t ks;
    vistrutah_256_key_expansion_fixed(key, 32, &ks, 10);
    ...
    for (int i = 0; i <= 5; i++) {
        ...
        if (i % 2 == 0) {
            ...
            printf("Variable key (k%d with RC[%d])\n", ((i/2) % 2), i);
        }
    }
}