// Vistrutah Block Cipher - Final Corrected Implementation
// Fixed timing of mixing layer in decryption
...
static void vistrutah_256_key_expansion_final(const uint8_t* key, int key_size,
    ...
    // Prepare inverse round keys for decryption
    // Apply inverse MixColumns to middle round keys
...
// Final Vistrutah-256 encryption
void vistrutah_256_encrypt_final(const uint8_t* plaintext, uint8_t* ciphertext,
                                const uint8_t* key, int key_size, int rounds) {
    vistrutah_256_state_t state;
    vistrutah_key_schedule_t ks;
    
    // Key expansion
    vistrutah_256_key_expansion_final(key, key_size, &ks, rounds);
...
// Final Vistrutah-256 decryption - FIXED
void vistrutah_256_decrypt_final(const uint8_t* ciphertext, uint8_t* plaintext,
                                const uint8_t* key, int key_size, int rounds) {
    vistrutah_256_state_t state;
    vistrutah_key_schedule_t ks;
    ...
    for (int round = rounds - 1; round >= 0; round--) {
        if (round == rounds - 1) {
            ...
            // Apply inverse mixing AFTER undoing the round where it was applied
            // Mixing was applied after rounds 2, 4, 6, ... during encryption
            // So we undo it after processing rounds 2, 4, 6, ...
...
void test_vistrutah_256_final() {
    printf("\n=== Testing Final Vistrutah-256 Implementation ===\n");
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        vistrutah_256_encrypt_final(plaintext, ciphertext, key, 32, rounds);
        vistrutah_256_decrypt_final(ciphertext, decrypted, key, 32, rounds);
